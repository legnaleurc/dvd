...
class SearchTest(IsolatedAsyncioTestCase):
    ...
    async def testInvalidateCacheByNode(self):
        ...
        self.assertFalse(
            SearchParam(name="CircleA (AuthorA)", fuzzy=True, parent_path=None, size=None) in cache
        )
        self.assertTrue(
            SearchParam(name="CircleB (AuthorB)", fuzzy=True, parent_path=None, size=None) in cache
        )
